...
class ControllerVideo():
    ...
    def next(self):
        ...
        if(not self.isFinished()):
            self.setFrame(self.conf['id_frame']+1)
    
    def getFrame(self):
        '''Pega o frame atual ou ultimo frame'''
        self.setFrame(self.conf["id_frame"])
        _, frame = self.conf['video'].read()
        return frame

    def setFrame(self, id_frame):
        '''Define um frame especifico aparttir do ID'''
        next_frame = id_frame  if id_frame <= self.conf['total_frame'] else self.conf['total_frame']
        self.conf['id_frame'] = next_frame
        self.conf['video'].set(cv2.CAP_PROP_POS_FRAMES, next_frame)
    ...
    def isFinished(self):
        ...
        self.conf['play'] = self.conf['id_frame'] <= self.conf['total_frame']
